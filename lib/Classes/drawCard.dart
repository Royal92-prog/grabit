...
class DrawCard extends StatelessWidget {
  ...
  DrawCard({required this.index, required this.playersNumber,
    ...
    required this.currentTurnCallback, required this.collectionName});
        ...
        if (currentTurn == -1 || currentTurn == -3) {
          ...
          Future.delayed(Duration.zero, () async {
            ...
            if(playersNumber != 0) {
              ...
              await FirebaseFirestore.instance.collection(collectionName).doc(
  ...
  return GestureDetector(
    onTap: (currentTurn != index || index != deviceIndex) ? null : () async{
      ...
      await db.collection(collectionName).doc("game${gameNum}").set({
      ...
      await db.collection(collectionName).doc('game${gameNum}MSGS').set(
      ...
      await db.collection(collectionName).doc('game${gameNum}').
  ...
  handleSpecialCardNo0(index) async {
    ...
    for (int i = 0; i < playersNumber; i++) {
      ...
      if (cardsHandler[i][0].length > 0) {
        ...
        await FirebaseFirestore.instance.collection(collectionName).doc("game${gameNum}").set({
          'matchingCards': cardsGroupArray,
          'matchingColorCards' : cardsColorArray,
          'cardsActiveUniqueArray' : cardsActiveUniqueArray,
